...
def testGenerator(test_path, num_image=60, target_size=(512, 512), flag_multi_class=False, as_gray=True):
    ...
    for i in range(num_image):
        ...
        io.imsave(os.path.join(test_path, "{}.{}".format(i, suff)), img)
        img = io.imread(os.path.join(test_path,"{}.{}".format(i, suff)), as_gray=as_gray)
        if img.dtype == 'uint8':
            img = img / 255
...
def atlas_to_mask(atlas_path, im_binary, mask_input_path, mask_warped_path, mask_output_path, n, use_unet,
...
def applyMask(image_path, mask_path, save_path, segmented_save_path, mat_save, threshold, git_repo_base, bregma_list,
              atlas_to_brain_align, region_labels=True):
    """
